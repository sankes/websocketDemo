	...
	public void run() throws Exception {
		...
		try {
			...
			b.group(bossGroup, workerGroup)
					...
					// Channel。SimpleChatServerInitializer 继承自ChannelInitializer
					// 是一个特殊的处理类，他的目的是帮助使用者配置一个新的 Channel。也许你想通过增加一些处理类比如
					// SimpleChatServerHandler 来配置一个新的 Channel
					// 或者其对应的ChannelPipeline 来实现你的网络程序。当你的程序变的复杂时，可能你会增加更多的处理类到
					// pipline 上，然后提取这些匿名类到最顶层的类上。
					.childHandler(new WebsocketChatServerInitializer())
					// 5.你可以设置这里指定的 Channel 实现的配置参数。我们正在写一个TCP/IP 的服务端，因此我们被允许设置
					// socket 的参数选项比如tcpNoDelay 和 keepAlive。请参考 ChannelOption
					// 和详细的 ChannelConfig 实现的接口文档以此可以对ChannelOption 的有一个大概的认识。
					.option(ChannelOption.SO_BACKLOG, 128)
					// 6.option() 是提供给NioServerSocketChannel
					// 用来接收进来的连接。childOption() 是提供给由父管道 ServerChannel
			...
			// 绑定端口，开始接收进来的连接
			// 7.我们继续，剩下的就是绑定端口然后启动服务。这里我们在机器上绑定了机器所有网卡上的 8080 端口。当然现在你可以多次调用
			// bind() 方法(基于不同绑定地址)。
			...
			log.info("websocket关闭了.........................");
		}
	}
	public static void openWebsocket() throws Exception {
		int port=500;
